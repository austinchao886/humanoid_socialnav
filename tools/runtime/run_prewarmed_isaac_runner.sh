...
while true; do
  ...
  echo "[isaac-runner-service] starting generation=$generation task=$task asset_profile=$asset_profile log=$log_path"
  ...
  echo "[isaac-runner-service] generation=$generation exited rc=$runner_rc; prewarming next clean session in ${restart_delay_s}s"
