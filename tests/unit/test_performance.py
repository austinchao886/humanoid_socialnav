...
from motion_pipeline.core.latency import initialize_timing, record_stage
from motion_pipeline.core.performance import (
...
def test_offline_video_gate_ignores_legacy_no_video_samples(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "motion_pipeline.core.performance.LATENCY_BUDGETS",
        {
            "offline_video_output": {
                "kind": "max_duration_s",
                "value": 60.0,
...
def test_explicit_generation_run_group_can_supply_generation_gate(
    ...
    monkeypatch.setattr(
        "motion_pipeline.core.performance.GENERATION_STAGES", {"validation"}
    )
